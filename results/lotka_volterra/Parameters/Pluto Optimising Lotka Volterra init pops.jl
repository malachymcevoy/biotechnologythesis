...
# ╔═╡ e64b7870-27c6-4717-81ad-112f48a21ea6
begin
	using Plots, DifferentialEquations, Optim, OptimizationOptimJL, Optimization, Zygote, SciMLSensitivity
...
# ╔═╡ 38b7bd1e-f5f2-11ef-19bc-03148e8ae481
md"# Optimisation of a Lotka-Volterra system (initial populations)"

# ╔═╡ a4175afb-604c-461c-b8c0-80b9404e25e5
md"""
Given the end goal of this project is to apply optimisation techniques to the ribogenesis model (which consists of seven ODEs), it seemed reasonable to start by trying to apply such optimisation techniques to a simpler system of ODEs.

For example, our first goal could be to maximise the population of prey at each oscillation by changing the starting population of prey: 
"""

# ╔═╡ 73a48e10-16ba-45df-943c-013fdf797602
md"Define the function:"

# ╔═╡ ee7edab8-6293-4b15-a7fa-874f364ec3c3
md"""
To define an optimization problem, you need the objective function $f$ to be minimized over the domain of $u$, the collection of optimization variables:

$min_uf(u,p)$

The initial guess for the minimizer is $u_0$, an `Abstract Array` whose geometry matches the desired geometry of `u`.
"""

# ╔═╡ 5a917c6c-aa65-4381-839d-4f2946db3726
md"### Maximise prey population by changing initial prey"

# ╔═╡ 4c60a929-3dfb-43b2-ab53-c10f5a0caadc
md"##### Generate a function that returns the peak prey:"

# ╔═╡ deaacce6-4b8b-4b74-8925-14d92c99ac76
# ╠═╡ disabled = true
# ╠═╡ skip_as_script = true
#=╠═╡
function f(u,p)
...
# ╔═╡ 40df06ad-d2b4-4bac-86d1-280d6151ef1c
# ╠═╡ disabled = true
# ╠═╡ skip_as_script = true
#=╠═╡
println(-f(20.03, [.5, 1.0, 3.0, 1.0, 5.0]))
  ╠═╡ =#

# ╔═╡ cb385690-0b46-4d41-8f6c-6d589646eb8b
# ╠═╡ disabled = true
#=╠═╡
md"##### Optimize the fuction we've created by changing the initial prey population:"
  ╠═╡ =#

# ╔═╡ c45a565e-bf72-4835-a9ce-e469c9dc49d2
# ╠═╡ disabled = true
#=╠═╡
md"Set the parameters, initial guess for prey population and lower and upper bounds on prey populations:"
  ╠═╡ =#

# ╔═╡ 5dcfafb1-0bf5-4889-a3c5-94ec4110252b
# ╠═╡ disabled = true
#=╠═╡
begin
	p = [.5, 1.0, 3.0, 1.0, 5.0] # α, β, γ, δ, y0 (initial predator population)
	u0 = [10.0]
	lb = [1]
	ub = [20.0]
end
...
# ╔═╡ bbf61fea-3932-4214-989c-aaadcc5298c0
# ╠═╡ disabled = true
#=╠═╡
opt_func_prey = OptimizationFunction(f, AutoZygote())
  ╠═╡ =#

# ╔═╡ ab7eeb87-4e62-49d1-8da4-c64c9736acde
# ╠═╡ disabled = true
#=╠═╡
opt_prob_prey = OptimizationProblem(opt_func_prey, u0, p; lb=lb, ub=ub)
  ╠═╡ =#

# ╔═╡ 5bcf5271-dcc2-4540-af48-ac5a6331384e
# ╠═╡ disabled = true
# ╠═╡ skip_as_script = true
#=╠═╡
sol = solve(opt_prob_prey, BFGS())
  ╠═╡ =#

# ╔═╡ aa4166e3-85ed-4686-a816-6d0880639259
#=╠═╡
println(sol)
  ╠═╡ =#

# ╔═╡ 4c2c3c7e-a4a8-494b-9eb9-edfc6eef6eca
#=╠═╡
sol.original
  ╠═╡ =#

# ╔═╡ 2133bdf2-ed29-4987-a917-9d351a1d994f
# ╠═╡ disabled = true
#=╠═╡
md"##### Maximise the predator population by changing the initial value"
  ╠═╡ =#

# ╔═╡ f35302ae-1234-4c05-b6db-9b05e1ab1f88
# ╠═╡ disabled = true
#=╠═╡
function g(u,p)
	α, β, γ, δ = p
	x0 = p[5] # This is the optimization variable (initial predator population)
	y0 = u[1] # this is the fixed initial predator population
	tspan = (0.0, 50.0) # the time span for the simulation
...
# ╔═╡ 5395aa2b-dbdb-40a9-a9aa-a0d2fe7fd40a
# ╠═╡ disabled = true
#=╠═╡
begin
	p .= [.5, 1.0, 3.0, 1.0, 5.0] # α, β, γ, δ, x0 (initial prey population)
	u0 .= [5.0]
	lb .= [0.1]
	ub .= [50.0]
end
...
# ╔═╡ 14b9b21a-ebdb-4303-b0d9-7cd60b759638
#=╠═╡
opt_func_pred = OptimizationFunction(g, AutoZygote())
  ╠═╡ =#

# ╔═╡ e2d104ec-bf27-4ceb-a1da-133e1311a0c8
#=╠═╡
opt_prob_pred = OptimizationProblem(opt_func_pred, u0, p; lb=lb, ub=ub)
  ╠═╡ =#

# ╔═╡ 2bda1636-dc47-4778-91b6-dfcab8dae489
#=╠═╡
sol_pred = solve(opt_prob_pred, NelderMead())
  ╠═╡ =#

# ╔═╡ a493cb10-4ae3-4f5a-8eee-a6eb7610acfe
#=╠═╡
begin
...
# ╔═╡ b050406c-48de-4567-b26a-7b2bf650a005
...
manifest_format = "2.0"
project_hash = "6a072f749f42f9d9538a6907faa801439db083a3"
...
git-tree-sha1 = "e594d6f62a5fb6e05389670e339a3f92632f70e3"
